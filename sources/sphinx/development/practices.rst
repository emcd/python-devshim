...
.. currentmodule:: devshim.base

* Do not use ``assert`` statements except in unit tests. Assertions are not
  triggered in optimized compilations. Raise an exception to report invalid
  internal state.

* Never raise a non-package exception from inside the package. All exceptions
  should be fusions between the package's base ``Omniexception`` and the
  relevant Python standard exceptions.
